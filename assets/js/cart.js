const cartData = async () => {
  ...
  if (uCart) {
    uCart.items.map((v1) => {
      let pData = productData.find((v2) => v2.id === v1.productId);

      /* DOM structure preserved precisely to maintain childNodes navigation in calcTotal */
      print += `<div class="cart-item py-3 border-bottom d-flex flex-column flex-sm-row align-items-sm-center justify-content-between gap-3">
            <div class="d-flex align-items-center gap-3">
                <img src='admin/images/category_img/${pData.image[0]}' alt="${pData.name}" style="width: 80px; height: 95px; object-fit: cover; border-radius: 8px;">
                <div>
  ...
  localStorage.setItem("cartID", uCart ? uCart.id : "");
...
const calcTotal = () => {
  ...
  for (let i = 0; i < cartItemsContainer.childNodes.length; i++) {
    let price = parseInt(
      cartItemsContainer.childNodes[i].childNodes[1].childNodes[3].childNodes[3].childNodes[1].innerHTML,
    );
    let qtty = parseInt(
      cartItemsContainer.childNodes[i].childNodes[3].childNodes[1].childNodes[3].innerHTML,
    );
